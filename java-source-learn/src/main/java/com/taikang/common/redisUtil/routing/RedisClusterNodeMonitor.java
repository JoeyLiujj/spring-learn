package com.taikang.common.redisUtil.routing;
...
public class RedisClusterNodeMonitor implements Runnable {
	private Logger log = LoggerFactory.getLogger(RedisClusterNodeMonitor.class);
	...
	private void forceDetectingNode() {
		...
		for (int i = 0; i < threadCount; i++) {
			...
			boolean result = node.checkStatus();
			try {
			log.info("node....."+node.getNodeId()+",checkstatus="+result);
				if (result) {
		...
		if (faultNodes.size() > 0) { // 如果检查到错误的节点有变更，执行handleNodeChanged()方法
			synchronized (redisClusterNodes) {
				...
				log.info("Cluster Node List" + redisClusterNodes);
				log.info("bad Node List" + badNodes);
