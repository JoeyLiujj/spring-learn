...
//    @Pointcut("execution(* cn.joey.aop.xmlconfig.UserService.*(..)) throws java.io.IOException")
//    @Pointcut("within(cn.joey.aop.xmlconfig.UserService+)")  //within中使用的表达式必须是类型全限定名，不支持通配符
//    @Pointcut("this(cn.joey.aop.xmlconfig.UserService+)")   //this中使用的表达式必须是类型全限定名，不支持通配符
//    @Pointcut("target(cn.joey.aop.xmlconfig.UserService)")  //target中使用的表达式必须是类型全限定名，不支持通配符
...
//    @Pointcut("@within(java.lang.Deprecated)") //匹配所以持有指定注解类型内的方法；注解类型也必须是全限定类型
                                               // 必须是在目标对象上声明这个注解
    @Pointcut("@annotation(cn.joey.aop.annotationconfig.CustomAnnotation)") //匹配指定方法上有给定注解的所有方法。
